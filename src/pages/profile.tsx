...
const ProfileStatsSection = () => {
  ...
  const totalXp = useBoundStore((state) => state.xpToday());
  ;
  const league = "Bronze";
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="flex gap-2 rounded-2xl border-2 border-gray-200 p-2 md:gap-3 md:px-6 md:py-4">
          ...
          <div className="flex flex-col">
            <span className={["text-xl font-bold", streak === 0 ? "text-gray-400" : ""].join(" ")}>{streak}</span>
        ...
        <div className="flex gap-2 rounded-2xl border-2 border-gray-200 p-2 md:gap-3 md:px-6 md:py-4">
          <EmptyMedalSvg />
          <div className="flex flex-col">
            <span className={["text-xl font-bold", top3Finishes === 0 ? "text-gray-400" : ""].join(" ")}>{top3Finishes}</span>
...
const ProfileFriendsSection = () => {
  ...
  const following = ["David", "Eva"];

  const displayed = state === "FOLLOWING" ? following : followers;
  ...
  return (
    <section>
      <div className="mb-5 flex items-center justify-between">
        ...
        <Link
          href="/friends"
          className="rounded-xl border-2 border-b-4 border-blue-500 bg-blue-400 px-4 py-2 text-sm font-bold uppercase text-white transition hover:brightness-110"
        >
          Add Friends
        </Link>
      </div>
      <div className="rounded-2xl border-2 border-gray-200">
        <div className="flex">
          <button
            className={["flex w-1/2 items-center justify-center border-b-2 py-3 font-bold uppercase hover:border-blue-400 hover:text-blue-400", state === "FOLLOWING" ? "border-blue-400 text-blue-400" : "border-gray-200 text-gray-400"].join(" ")}
            onClick={() => setState("FOLLOWING")}
          >
            Following
          </button>
          <button
            className={["flex w-1/2 items-center justify-center border-b-2 py-3 font-bold uppercase hover:border-blue-400 hover:text-blue-400", state === "FOLLOWERS" ? "border-blue-400 text-blue-400" : "border-gray-200 text-gray-400"].join(" ")}
          ...
          {displayed.length > 0 ? (
            <ul className="w-full px-6">
              {displayed.map((name) => (
                <li
                  key={name}
                  className="flex w-full items-center justify-between border-b border-gray-100 py-4 text-base"
                >
                  <span>{name}</span>
            ...
            <span>No {state.toLowerCase()} yet</span>
          )}
        </div>
      </div>
    </section>
