...
const getTileColors = ({
  ...
  switch (status) {
    case "LOCKED":
      ...
      return "border-gray-600 bg-gray-800"; // Darker locked tile
    case "COMPLETE":
      return "border-orange-500 bg-orange-400"; // LeetCode orange for completed tiles
    case "ACTIVE":
...
const TileTooltip = ({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("click", containsTileTooltip, true);
  }, [selectedTile, tileTooltipRef, closeTooltip, index]);

  const unit = units.find((unit) => unit.unitNumber === unitNumber);
  const activeBackgroundColor = unit?.backgroundColor ?? "bg-gray-700"; // Darker active background
  const activeTextColor = unit?.textColor ?? "text-orange-400"; // LeetCode orange for text
      ...
      >
        <div
          className={[
            ...
            status === "ACTIVE"
              ? activeBackgroundColor
              : status === "LOCKED"
              ? "border-l-2 border-t-2 border-gray-600 bg-gray-800"
              : "bg-orange-400",
          ].join(" ")}
          style={{
        ...
        {status === "ACTIVE" ? (
          <Link
            href="/lesson"
            className={[
              "flex w-full items-center justify-center rounded-xl border-b-4 border-gray-600 bg-gray-700 p-3 uppercase",
              activeTextColor,
            ].join(" ")}
          >
          ...
          <button
            className="w-full rounded-xl bg-gray-800 p-3 uppercase text-gray-400"
            disabled
          >
            Locked
          </button>
          ...
          <Link
            href="/lesson"
            className="flex w-full items-center justify-center rounded-xl border-b-4 border-orange-500 bg-gray-700 p-3 uppercase text-orange-400"
          >
            Practice +5 XP
          </Link>
        )}
  ...
  return (
    <article
      className={[
        ...
        "bg-gray-900", // Darker header background
      ].join(" ")}
    >
